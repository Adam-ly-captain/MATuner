...
if __name__ == "__main__":
    ...
    episode_data = pd.read_excel("./model/maddpg_src/res/res.xlsx")
    ...
    for i in range(episode):
        ...
        episode_data.to_excel("./model/maddpg_src/res/res.xlsx", index=False)
    ...
    # state = [state1, state2]
    # action1 = torch.randn(batch_size, action_dim)
    # action2 = torch.randn(batch_size, action_dim)
    # action = torch.cat([action1, action2], 1)
    # reward = torch.randn(batch_size, 1)
    # next_state1 = torch.randn(batch_size, state_dim)
    # next_state2 = torch.randn(batch_size, state_dim)
    # next_state = [next_state1, next_state2]

    # # Create an instance of MADDPG
    # maddpg = MADDPG(
    #     agent_num,
    #     state_dim,
    #     action_dim,
    ...
    # maddpg.iteratively_train(state, action, reward, next_state)

    # maddpg.mapper_knobs(maddpg.get_action(state))
