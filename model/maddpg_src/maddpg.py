...
from model.maddpg_src.experience_replay import ExperienceBuffer
...
class MADDPG:
    ...
    def save_model(self, path="./model/maddpg_src/res/"):
        """
    ...
    def load_model(self, path="./model/maddpg_src/res/"):
        """
    ...
    def load_experience(self, path="./model/maddpg_src/res/"):
        """
        加载经验回放
        """
        for i in range(self.agent_num):
    ...
    def train(self, i, state, action, reward, next_state):
        ...
        actor_loss = self.update_actor(i, state, action)
    ...
    def update_actor(self, i, state, action):
        """
        policy update
        """
        # 计算actor loss
        action = [action[j] if j != i else self.actors[i](state[i]) for j in range(self.agent_num)]
