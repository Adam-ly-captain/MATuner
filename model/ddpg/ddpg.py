...
from model.ddpg.experience_replay import ExperienceBuffer
...
class DDPG:
    ...
    def __init__(
        ...
        self.critic_state_dim = state_dim  # 单个agent的state
        self.critic_action_dim = action_dim  # 单个agent的action
    ...
    def init_model(self):
        self.actors = Actor(self.actor_state_dim, self.actor_action_dim)
        self.actors_target = Actor(self.actor_state_dim, self.actor_action_dim)

        self.critics = Critic(self.critic_state_dim, self.critic_action_dim)
        self.critics_target = Critic(self.critic_state_dim, self.critic_action_dim)

        self.actor_optimizers = optimizers.Adam(
            self.actors.parameters(), lr=self.actor_lr
        )
        self.critic_optimizers = optimizers.Adam(
            self.critics.parameters(), lr=self.critic_lr
        )

        Log().Debug("DDPG: Initialized")
    ...
    def hard_update(self):
        ...
        self.actors_target.load_state_dict(self.actors.state_dict())
    ...
    def soft_update(self):
        ...
        for param, target_param in zip(
            self.actors.parameters(), self.actors_target.parameters()
        ):
    ...
    def save_model(self, path="./model/ddpg/res/"):
        """
        保存main network和target network
        """
        torch.save(self.actors.state_dict(), path + f"actor_{self.node_id}.pth")
        torch.save(self.critics.state_dict(), path + f"critic_{self.node_id}.pth")
        torch.save(
            self.actors_target.state_dict(), path + f"actor_target_{self.node_id}.pth"
        )
        torch.save(
            self.critics_target.state_dict(), path + f"critic_target_{self.node_id}.pth"
        )
        self.buffer.save(path=path, filename=f"experience_{self.node_id}.npz")

        Log().Info(f"save ddpg model and experience replay to {path} successfully")

    def load_model(self, path="./model/ddpg/res/"):
        """
        加载main network和target network
        """
        self.actors.load_state_dict(torch.load(path + f"actor_{self.node_id}.pth"))
        self.critics.load_state_dict(torch.load(path + f"critic_{self.node_id}.pth"))
        self.actors_target.load_state_dict(
            torch.load(path + f"actor_target_{self.node_id}.pth")
        )
        self.critics_target.load_state_dict(
            torch.load(path + f"critic_target_{self.node_id}.pth")
        )

        Log().Info(f"load ddpg model from {path} successfully")

    def load_experience(self, path="./model/ddpg/res/"):
        """
        加载经验回放
        """
        self.buffer.load(path=path, filename=f"experience_{self.node_id}.npz")
    ...
    def iteratively_train(self, state, action, reward, next_state):
        ...
        迭代训练
        """
        critic_loss, actor_loss = self.train(state, action, reward, next_state)
    ...
    def train(self, state, action, reward, next_state):
        ...
        self.actors.remove_noise()
        self.actors_target.remove_noise()

        critic_loss = self.update_critic(state, action, reward, next_state)
        actor_loss = self.update_actor(state)

        Log().Debug(f"train agent {self.node_id} successfully")
    ...
    def update_actor(self, state):
        ...
        """
        # 计算actor loss
        action = self.actors(state)
        # action = torch.cat(action, 1)
        # state = torch.cat(state, 1)
        actor_loss = -self.critics(state, action).mean()
    ...
    def get_action(self, state, noise_flag=True):
        """
        noise: behavior policy get action
        """
        if noise_flag:
            self.actors.sample_noise()
        else:
            self.actors.remove_noise()

        action = self.actors(state)[0].unsqueeze(0)
        return action
    ...
    def mapper_knobs(self, action):
        ...
        continuous_knobs = self.process_continuous_knobs(
            action[:, : self.continuous_knobs_num]
        )
        discrete_knobs = self.process_discrete_knobs(
            action[:, self.continuous_knobs_num :]
        )
        res_knobs = torch.cat([continuous_knobs, discrete_knobs], 1)
    ...
    def process_discrete_knobs(self, action):
        ...
        for i, knob in enumerate(self.knobs["discrete"]):
            ...
            dis_action[:, i] = torch.clamp(
                torch.floor((action[:, i] + 1) / bin_width), 0, num_buckets - 1
            )

        return dis_action

    def get_episode(self):
        return self.buffer.episode
