...
class FACMACLearner:
    def __init__(self, mac, scheme, args, logger, metrics_logger):
        self.args = args
        self.logger = logger
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        self.logger.console_logger.info(f"Reward: {rewards}, Actions: {actions}")
        # print(rewards, actions, terminated, mask)

        # Train the critic batched
        target_actions = []
        self.target_mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_target_outs, origin_agent_target_outs = self.target_mac.select_actions(batch, t_ep=t, t_env=None, test_mode=True,
                                                               critic=self.target_critic, target_mac=True)
            target_actions.append(origin_agent_target_outs)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.metrics_logger['t'] = t_env
            self.metrics_logger['criticloss'] = loss.item()
            self.metrics_logger['critic_grad_norm'] = critic_grad_norm
            self.metrics_logger['target_mean'] = targets.sum().item() / mask_elems
            self.metrics_logger['actorloss'] = pg_loss.item()
            self.metrics_logger['agent_grad_norm'] = agent_grad_norm
